...
final reportStatsProvider = FutureProvider<Map<String,dynamic>>((ref) =>
  ref.read(firebaseServiceProvider).getDashboardStats());
      ...
      body: stats.when(
        ...
        data: (d) => ListView(padding: const EdgeInsets.all(16), children: [
          GridView.count(crossAxisCount: 2, crossAxisSpacing: 12, mainAxisSpacing: 12, childAspectRatio: 1.4, shrinkWrap: true, physics: const NeverScrollableScrollPhysics(), children: [
            _Card('💰', 'المقبوضات', '\${fmt.format(d['totalCollected']??0)} ر.س', AppColors.accent),
            _Card('📋', 'الديون', '\${fmt.format(d['totalDebtAmount']??0)} ر.س', AppColors.primaryLight),
            _Card('⚠️', 'المتأخرات', '\${fmt.format(d['totalOverdue']??0)} ر.س', AppColors.danger),
            _Card('👥', 'العملاء', '\${d['totalCustomers']??0}', AppColors.primary),
          ]),
          ...
          Card(child: Padding(padding: const EdgeInsets.all(16), child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
            ...
            SizedBox(height: 160, child: BarChart(BarChartData(
              ...
              barGroups: [8000,12000,9000,15000,11000,18000].asMap().entries.map((e) =>
                BarChartGroupData(x: e.key, barRods: [BarChartRodData(toY: e.value.toDouble(), color: AppColors.primaryLight, width: 22, borderRadius: BorderRadius.circular(6))])).toList(),
            ))),
          ]))),
