...
final customerDebtsProvider = StreamProvider.family<List<Map<String,dynamic>>, String>(
  (ref, id) => ref.read(firebaseServiceProvider).stream('debts',
    where: [['customerId', id]], orderBy: 'createdAt', descending: true));
final customerPaymentsProvider = StreamProvider.family<List<Map<String,dynamic>>, String>(
  (ref, id) => ref.read(firebaseServiceProvider).stream('payments',
    where: [['customerId', id]], orderBy: 'paymentDate', descending: true, limit: 20));
...
class _State extends ConsumerState<CustomerDetailScreen> with SingleTickerProviderStateMixin {
        ...
        return Scaffold(
          ...
          body: Column(children: [
            Container(
              ...
              child: Column(children: [
                ...
                Row(children: [
                  ...
                  _S(fmt.format(remaining) + ' ر.س', 'المتبقي',
                    remaining > 0 ? const Color(0xFFFFD3B6) : const Color(0xFFA8E6CF)),
                ]),
            ...
            Padding(padding: const EdgeInsets.all(12), child: Row(children: [
              _Btn('دين جديد', Icons.add_card_outlined, AppColors.accent,
                () => context.push('/debts/add?customerId=${widget.customerId}')),
              const SizedBox(width: 8),
              _Btn('دفعة', Icons.payments_outlined, AppColors.warning,
  ...
  Widget _Btn(String l, IconData icon, Color c, VoidCallback f) => Expanded(
    child: GestureDetector(onTap: f,
      child: Container(
        ...
        child: Column(children: [
          ...
          Text(l, style: TextStyle(fontSize: 10, color: c, fontWeight: FontWeight.w700)),
        ]),
      ),
    ),
  );
                ...
                trailing: Column(mainAxisAlignment: MainAxisAlignment.center, children: [
                  Container(
                    ...
                    child: Text(status.statusLabel,
                      style: TextStyle(color: status.statusColor, fontSize: 11, fontWeight: FontWeight.w700)),
                  ),
                  const SizedBox(height: 4),
                  Text(fmt.format(total - paid) + ' ر.س',
                    style: TextStyle(
                      color: (total-paid) > 0 ? AppColors.warning : AppColors.accent,
                      fontWeight: FontWeight.w800, fontSize: 12)),
                ]),
                onTap: () => context.push('/debts/$did'),
              ));
            },
          ),
    );
