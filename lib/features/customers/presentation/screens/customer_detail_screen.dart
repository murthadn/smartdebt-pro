...
final customerDetailProvider = FutureProvider.family<Map<String,dynamic>?, String>(
  (ref, id) => ref.read(firebaseServiceProvider).get('customers', id));
final customerDebtsProvider = StreamProvider.family<List<Map<String,dynamic>>, String>(
  (ref, id) => ref.read(firebaseServiceProvider).stream('debts', where: [['customerId', id]], orderBy: 'createdAt', descending: true));
final customerPaymentsProvider = StreamProvider.family<List<Map<String,dynamic>>, String>(
  (ref, id) => ref.read(firebaseServiceProvider).stream('payments', where: [['customerId', id]], orderBy: 'paymentDate', descending: true, limit: 20));
...
class _State extends ConsumerState<CustomerDetailScreen> with SingleTickerProviderStateMixin {
  ...
  Widget build(BuildContext context) {
    ...
    return c.when(
      ...
      error: (e,_) => Scaffold(body: Center(child: Text('خطأ'))),
      data: (cust) {
        ...
        return Scaffold(
          ...
          body: Column(children: [
              ...
              child: Column(children: [
                Row(children: [
                  Container(width: 56, height: 56,
                    ...
                    child: Center(child: Text(name.isNotEmpty ? name.characters.first : '?',
                ...
                ClipRRect(borderRadius: BorderRadius.circular(4),
                  child: LinearProgressIndicator(value: progress, minHeight: 7, backgroundColor: Colors.white24, color: Colors.white)),
              ])),
            Padding(padding: const EdgeInsets.all(12), child: Row(children: [
              ...
              if (phone != null) ...[
                const SizedBox(width: 8),
                _Btn('واتساب', Icons.chat_outlined, const Color(0xFF25D366), () {
                  final n = phone.replaceAll(RegExp(r'[^0-9]'),'');
                  final intl = n.startsWith('0') ? '966\${n.substring(1)}' : n;
                  launchUrl(Uri.parse('https://wa.me/\$intl'));
                }),
              ],
            ])),
  ...
  Widget _Btn(String l, IconData icon, Color c, VoidCallback f) => Expanded(child: GestureDetector(onTap: f,
    child: Container(padding: const EdgeInsets.symmetric(vertical: 10),
      ...
      child: Column(children: [Icon(icon, color: c, size: 20), const SizedBox(height: 3), Text(l, style: TextStyle(fontSize: 10, color: c, fontWeight: FontWeight.w700))]))));
}
...
class _DebtsTab extends ConsumerWidget {
  ...
  @override
  Widget build(BuildContext context, WidgetRef ref) => ref.watch(customerDebtsProvider(customerId)).when(
            ...
            return Card(child: ListTile(
              ...
              subtitle: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
                ...
                ClipRRect(borderRadius: BorderRadius.circular(3),
                  child: LinearProgressIndicator(value: total > 0 ? (paid/total).clamp(0.0,1.0) : 0, minHeight: 5, color: status.statusColor, backgroundColor: Colors.grey.withOpacity(0.2))),
              ]),
              trailing: Column(mainAxisAlignment: MainAxisAlignment.center, children: [
                ...
                const SizedBox(height: 4),
                Text(fmt.format(total-paid) + ' ر.س',
                  style: TextStyle(color: (total-paid) > 0 ? AppColors.warning : AppColors.accent, fontWeight: FontWeight.w800, fontSize: 12)),
              ]),
...
class _PaysTab extends ConsumerWidget {
  ...
  @override
  Widget build(BuildContext context, WidgetRef ref) => ref.watch(customerPaymentsProvider(customerId)).when(
    loading: () => const Center(child: CircularProgressIndicator()),
    error: (e,_) => Center(child: Text('خطأ')),
      ...
      : ListView.separated(padding: const EdgeInsets.all(12), itemCount: list.length,
          ...
          itemBuilder: (_,i) {
            ...
            return Card(child: ListTile(
              ...
              title: Text(fmt.format(p['amount']??0) + ' ر.س',
