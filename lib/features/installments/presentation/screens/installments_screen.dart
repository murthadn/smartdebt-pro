...
class InstallmentsScreen extends ConsumerWidget {
  ...
  Widget build(BuildContext context, WidgetRef ref) {
    final stream = ref.watch(StreamProvider((r) =>
      r.read(firebaseServiceProvider).stream('installments', orderBy: 'dueDate')));
              ...
              itemBuilder: (_,i) {
                ...
                return Card(child: ListTile(
                  leading: Container(width: 36, height: 36,
                    decoration: BoxDecoration(color: status.statusColor.withOpacity(0.15), borderRadius: BorderRadius.circular(10)),
                    child: Center(child: Text('\${inst['installmentNo']??1}',
                      style: TextStyle(fontWeight: FontWeight.w800, color: status.statusColor)))),
                  title: Text(inst['customerName']??'', style: const TextStyle(fontWeight: FontWeight.w700)),
                  subtitle: Text('استحقاق: \$dueDate', style: const TextStyle(fontSize: 12)),
                  trailing: Column(mainAxisAlignment: MainAxisAlignment.center, children: [
                    Text(fmt.format(amount) + ' ر.س', style: TextStyle(fontWeight: FontWeight.w800, color: status.statusColor, fontSize: 14)),
