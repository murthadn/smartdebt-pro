...
final debtInstallmentsProvider = StreamProvider.family<List<Map<String,dynamic>>, String>(
  (ref, id) => ref.read(firebaseServiceProvider).stream(
    'installments', where: [['debtId', id]], orderBy: 'installmentNo'));
      ...
      appBar: AppBar(
        ...
        actions: [
          IconButton(
            ...
            onPressed: () => context.push('/payments/add?debtId=$debtId'),
          ),
        ],
      ),
      body: debt.when(
                ...
                Row(children: [
                  ...
                  Container(
                    ...
                    decoration: BoxDecoration(
                      color: status.statusColor.withOpacity(0.25),
                      borderRadius: BorderRadius.circular(20),
                    ),
                    child: Text(status.statusLabel,
                      style: TextStyle(color: status.statusColor, fontWeight: FontWeight.w700, fontSize: 12)),
                  ),
                ]),
                Text(d['customerName'] ?? '',
                        ...
                        trailing: instStatus != 'paid'
                          ? TextButton(
                              onPressed: () => context.push(
                                '/payments/add?debtId=$debtId&installmentId=$instId'),
